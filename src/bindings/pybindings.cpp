...
PYBIND11_MODULE(_score_native, m) {
    ...
    py::class_<score::Series<double>>(m, "Series")
        ...
        .def(py::init<std::vector<double>, std::vector<std::string>>(), py::arg("data"),
             py::arg("dates"), "Construct from values and matching ISO date strings.")
        ...
        .def(
            "__iter__",
            [](const score::Series<double>& s) { return py::make_iterator(s.begin(), s.end()); },
            py::keep_alive<0, 1>())
        .def("size", &score::Series<double>::size)
        .def("empty", &score::Series<double>::empty)
        .def("values", &score::Series<double>::values, py::return_value_policy::reference_internal)
        .def("dates", &score::Series<double>::dates, py::return_value_policy::reference_internal)
    ...
    py::class_<score::statistics::DescriptiveStats>(m, "DescriptiveStats")
        .def(py::init<const score::Series<double>&>(), py::arg("series"), py::keep_alive<1, 2>(),
             "Construct from a Series.  The Series must outlive this object.")
        .def("mean", &score::statistics::DescriptiveStats::mean)
        .def("variance", &score::statistics::DescriptiveStats::variance, py::arg("sample") = true)
        .def("stddev", &score::statistics::DescriptiveStats::stddev, py::arg("sample") = true)
        ...
        .def("quantile", &score::statistics::DescriptiveStats::quantile, py::arg("q"))
    ...
    py::class_<score::metrics::StockAnalyzer>(m, "StockAnalyzer")
        .def(py::init<const score::Series<double>&>(), py::arg("prices"), py::keep_alive<1, 2>(),
        ...
        .def("annualized_volatility", &score::metrics::StockAnalyzer::annualized_volatility,
             py::arg("periods_per_year") = 252)
        .def("sharpe_ratio", &score::metrics::StockAnalyzer::sharpe_ratio,
             py::arg("risk_free_rate") = 0.0, py::arg("periods_per_year") = 252)
