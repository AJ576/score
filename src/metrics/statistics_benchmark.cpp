...
void validate_wiener(double check_time, double time_step, int n_paths)
{
    ...
    for (double s : samples) sum_sq_dev += (s - sample_mean) * (s - sample_mean);
    ...
    for (double s : samples)
    {
        ...
        const double u = my_math::cdf(s, 0.0, expected_stddev); // should be ~Uniform(0,1) if s really is normal

        int bin = static_cast<int>(u * n_bins);
        if (bin == n_bins) bin -= 1; // guard u==1.0 edge case landing out of range
        bin_counts[bin]++;
    }
