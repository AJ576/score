...
void run_benchmark(const std::string &name, F&& f, const std::vector<double>& inputs, int iterations)
{
    ...
    for (double x: inputs)
    {
        ...
        for (int i = 0; i < warmup_iterations; i++)
            ...
            sink = f(x+i*1e-15); //technincally overwrites but averaging out is just gonna lose precision so whatever. 
...
int main()
{
    const std::vector<double> inputs = {
        -37, -30, -15, -10, -8.5, -7.5, -5, -2, -1, -0.5, -0.05, 0, 0.05, 0.5, 1, 2, 5, 7.5, 8.5, 10
    };
