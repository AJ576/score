...
inline constexpr std::array<double, 8> Q = {
...
[[nodiscard]] inline double my_erfc(double a) noexcept {
    ...
    const auto [p, q] = x < 8.0 ? std::pair{detail::polevl(x, detail::P), detail::p1evl(x, detail::Q)}
                                 : std::pair{detail::polevl(x, detail::R), detail::p1evl(x, detail::S)}; // its better to have p and q as a pair, instead of their own variables
